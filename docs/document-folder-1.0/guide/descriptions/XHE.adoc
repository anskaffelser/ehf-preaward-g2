...
<5> The specification identifier (`CustomizationID`) of the Payload instance
<6> The business process identifier (`ProfileID`) of the Payload instance
<7> Payload instance is not encrypted- always use `False`
<8> The payload content of each structured document
<9> Structured information of the specific business document (XML based), here EHF Catalogue 3.0


.Example of how to implement multiple payloads in XHE.
[source, xml, indent=0]
----
include::../{snippet-dir}/document-folder-multiple.xml[tags=multiple-payloads]
----
<1> Payload number 1, here EHF Catalogue 3.0
<2> Payload number 2, here PEPPOL BIS Billing 3.0
<3> Payload number 2, here EHF Order 3.0
