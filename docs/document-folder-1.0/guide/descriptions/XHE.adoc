...
To send EHF Document Folder following metadata information need to be provided (illustrated in Figure (3)):
...
receivers organization numbers.
...
<7> Senders organization number, where `schemeID=0192`
<8> Receivers organization number, where `schemeID=0192`
...
For example, the payload instance can be catalogue template, or other structured or non-structured
tender documents.
...
<4> Type of document expressed as a code from links:https://test-vefa.difi.no/ehf-pre/g2/codelist/UNCL1001/[code list]
